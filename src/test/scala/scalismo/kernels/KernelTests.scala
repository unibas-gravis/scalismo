...
import scalismo.geometry.{ Point, SpatialVector, _1D, _3D }
...
class KernelTests extends ScalismoTestSuite {
  ...
  describe("A sample covariance kernel") {
    it("can reproduce the covariance function from random samples") {
      ...
      val mu = (pt: Point[_3D]) => SpatialVector(1, 10, -5)
      val gp = LowRankGaussianProcess.approximateGP[_3D, SpatialVector[_3D]](GaussianProcess(Field(domain, mu), k), samplerForNystromApprox, 500)

      val sampleTransformations = for (i <- (0 until 5000)) yield {
        // TODO: gp.sample() should (arguably) accept seed.
        val sample: (Point[_3D] => SpatialVector[_3D]) = gp.sample()
