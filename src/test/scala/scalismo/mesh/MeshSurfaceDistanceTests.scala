...
class MeshSurfaceDistanceTests extends ScalismoTestSuite {
  ...
  def randomVector(offset: Double = 0.0, scale: Double = 1.0): SpatialVector[_3D] = {
    SpatialVector(rgen(offset, scale), rgen(offset, scale), rgen(offset, scale))
  }
  ...
  def aeqV[D <: Dim](a: SpatialVector[D], b: SpatialVector[D], theta: Double = 1.0e-8): Boolean = {
  ...
  describe("The BoundingSphere") {
    ...
    it("should find the correct closest points pairs in a sorted list") {
      ...
      def bruteForcePairFinder(sortedPoints: IndexedSeq[(SpatialVector[_3D], Int)]) = {
