...
class TransformationTests extends ScalismoTestSuite {
  ...
  describe("A translation in 2D") {
    ...
    it("translates a 1D image") {
      ...
      val continuousImage = DifferentiableScalarImage(domain.boundingBox, (x: Point[_1D]) => (x * x), (x: Point[_1D]) => SpatialVector(2f * x))
  ...
  describe("A 2D similarity transform") {
    ...
    val rigidTransformation = RigidTransformation(TranslationTransform(SpatialVector2D(2.0, 5.0)), RotationTransform(Math.PI / 2.0, Point2D(0, 0)))
  ...
  describe("A 3D similarity transform") {
    ...
    val rigidTransformation = RigidTransformation(TranslationTransform(SpatialVector3D(2.0, 5.0, 1.0)), RotationTransform(Math.PI / 2.0, 0.0, 0.0, Point3D(0, 0, 0)))
