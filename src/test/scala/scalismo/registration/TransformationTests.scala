...
class TransformationTests extends ScalismoTestSuite {
  ...
  describe("In 3D") {
    ...
    it("rotation is invertible on meshes") {
      ...
      rotRotMesh.domain.points.zipWithIndex.foreach {
        case (p, i) =>
          val id = PointId(i)
          p(0) should be(mesh.domain.point(id)(0) +- 0.000001)
          p(1) should be(mesh.domain.point(id)(1) +- 0.000001)
          p(2) should be(mesh.domain.point(id)(2) +- 0.000001)
      }
    }
    ...
    it("a rigid transformation yields the same result as the composition of rotation and translation") {
      ...
      val diffNormMax = transformedRigid.domain.points.zip(transformedComposed.domain.points).map { case (p1, p2) => (p1 - p2).norm }.max
