...
import scalismo.common.{Field, RealSpace, VectorField, Vectorizer}
...
object main {
  def main(args: Array[String]): Unit = {
    ...
    val tetramesh = TetraMeshIO.readTetraMesh(new File("E:\\PhD folders\\Tetrahedral mesh\\test file\\SW9-040M-RS-grid.inp")).get
...
import scalismo.utils.Random.implicits._
  ...
  def imagepointTotetrahedron(p: Point[_3D],tetramesh:TetrahedralMesh[_3D] ,image:DiscreteScalarImage[_3D,Short],
                              l: IndexedSeq[((Seq[Point[_3D]],
                                scala.IndexedSeq[(Point[_3D], Short)]),
                                DenseVector[Double])]):(Int,IndexedSeq[Point[_3D]],DenseVector[Double])={
