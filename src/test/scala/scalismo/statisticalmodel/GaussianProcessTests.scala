...
class GaussianProcessTests extends ScalismoTestSuite {
  ...
  describe("samples from a gaussian process") {
    ...
    def testVarianceForGP(gp: GaussianProcess[_1D, SpatialVector[_1D]], domain: BoxDomain[_1D]): Unit = {
      val numPoints = 3
    ...
    it("have the correct mean and variance for a full gp") {
      ...
      val m = Field(domain, (_: Point[_1D]) => SpatialVector(0))
      val k = DiagonalKernel(GaussianKernel[_1D](2.0), 1)
      val gp = GaussianProcess[_1D, SpatialVector[_1D]](m, k)
      testVarianceForGP(gp, domain)
    }

    it("have the correct mean and variance for a low-rank gp") {
      val domain = BoxDomain(Point(-1.0), Point(1.0))
      val m: Field[_1D, SpatialVector[_1D]] with Object { val f: (Point[_1D]) => SpatialVector[_1D]; def domain: Domain[_1D] } = Field(domain, (_: Point[_1D]) => SpatialVector(0))
    ...
    it("yields the same values as the original gp at the given points") {
      ...
      val m = Field(domain, (_: Point[_2D]) => SpatialVector(0.0f, 0.0f))
  ...
  describe("A Gaussian process regression") {
    it("keeps the landmark points fixed for a 1D case") {
      ...
      val gp = GaussianProcess(Field(domain, (_: Point[_1D]) => SpatialVector(0f)), kernel)
      val gpLowRank = LowRankGaussianProcess.approximateGP(gp, UniformSampler(domain, 500), 100)

      val trainingData = IndexedSeq((-3.0, 1.0), (-1.0, 3.0), (0.0, -1.0), (1.0, -1.0), (3.0, 0.0)).map(t => (Point(t._1), SpatialVector(t._2)))
    ...
    it("yields a larger posterior variance for points that are less strongly constrained") {
      ...
      val trainingData = IndexedSeq((pt1, val1, 0.1), (pt2, val2, 2.0))
        .map(t => (Point(t._1), SpatialVector(t._2), errorForSigma(t._3)))
    ...
    it("keeps the landmark points fixed for a 2D case") {
      ...
      val gp = GaussianProcess[_2D, SpatialVector[_2D]](DiagonalKernel(GaussianKernel[_2D](5), 2))
      val gpLowRank = LowRankGaussianProcess.approximateGP[_2D, SpatialVector[_2D]](gp, UniformSampler(domain, 400), 100)

      val trainingData = IndexedSeq((Point(-3.0, -3.0), SpatialVector(1.0, 1.0)), (Point(-1.0, 3.0), SpatialVector(0.0, -1.0)))
    ...
    it("keeps the landmark points fixed for a 3D case") {
      ...
      val gp = GaussianProcess[_3D, SpatialVector[_3D]](DiagonalKernel(GaussianKernel[_3D](5), 3))
      val gpLowRank = LowRankGaussianProcess.approximateGP[_3D, SpatialVector[_3D]](gp, UniformSampler(domain, 6 * 6 * 6), 50)

      val trainingData = IndexedSeq((Point(-3.0, -3.0, -1.0), SpatialVector(1.0, 1.0, 2.0)), (Point(-1.0, 3.0, 0.0), SpatialVector(0.0, -1.0, 0.0)))
      val posteriorGPLowRank = gpLowRank.posterior(trainingData, 1e-5)
      val posteriorGP = gp.posterior(trainingData, 1e-5)
  ...
  describe("a Gaussian process marginal likelihood") {
    ...
    object Fixture {
      ...
      val moreComplexGP = GaussianProcess[_3D, SpatialVector[_3D]](DiagonalKernel(GaussianKernel[_3D](5) + GaussianKernel[_3D](2.5) * 2 + GaussianKernel[_3D](0.5) * 1, 3))
      ...
      val dataOnMean = discreteDomain.points.toIndexedSeq.map { p => (p, SpatialVector(0.0, 0.0, 0.0), littleNoise) }
    }
  ...
  describe("a lowRankGaussian process") {
    object Fixture {
      ...
      val gp = {
        LowRankGaussianProcess.approximateGP[_3D, SpatialVector[_3D]](GaussianProcess(Field(domain, (_: Point[_3D]) => SpatialVector(0.0, 0.0, 0.0)), kernel), sampler, 200)
      }
    }
    ...
    it("yields the same covariance as given by the kernel for a real matrix valued kernel (with nondiagonal block structure)") {
      ...
      val gp = {
        LowRankGaussianProcess.approximateGP[_3D, SpatialVector[_3D]](GaussianProcess(Field(domain, (_: Point[_3D]) => SpatialVector(0.0, 0.0, 0.0)), kernel), sampler, 5)
      }
    ...
    it("can be discretized and yields the correct values at the discretization points") {
      ...
      val mean = Field[_3D, SpatialVector[_3D]](RealSpace[_3D], _ => SpatialVector(0.0, 0.0, 0.0))
  ...
  describe("a discrete LowRank Gaussian process") {
    ...
    object Fixture {
      ...
      val trainingData = IndexedSeq((0, SpatialVector.zeros[_3D]), (discretizationPoints.size / 2, SpatialVector.zeros[_3D]), (discretizationPoints.size - 1, SpatialVector.zeros[_3D])).map { case (i, v) => (PointId(i), v) }
    ...
    it("yeilds the same result when marginalized the points in one or two steps") {
      ...
      DiscreteField.vectorize[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]](dgp2.mean) should equal(DiscreteField.vectorize[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]](dgp3.mean))
    ...
    it("yields the same result for the marginal as the discrete gp") {
      ...
      DiscreteField.vectorize[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]](dgp1.mean) should equal(DiscreteField.vectorize[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]](dgp2.mean))
