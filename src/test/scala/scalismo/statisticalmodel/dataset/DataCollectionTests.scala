...
class DataCollectionTests extends ScalismoTestSuite {
  ...
  describe("A datacollection") {
    ...
    val transformations = for (i <- 0 until 10) yield TranslationTransform(SpatialVector(i.toDouble, 0.0, 0.0))
  ...
  describe("Generalization") {
    ...
    val zeroMean = Field(Fixture.dc.reference.boundingBox, (pt: Point[_3D]) => SpatialVector(0, 0, 0))
    val matrixValuedGaussian = DiagonalKernel(GaussianKernel[_3D](25) * 20, 3)
    val bias: GaussianProcess[_3D, SpatialVector[_3D]] = GaussianProcess(zeroMean, matrixValuedGaussian)
