...
class IntegrationTest extends FunSpec with ShouldMatchers {
  ...
  describe("An integration in 1D") {
    it("Correctly integrates x squared on interval [-1,1]") {
      ...
      val img = ContinuousScalarImage1D(BoxDomain[_1D](0f, 1.0f), (x: Point[_1D]) => x * x, Some((x: Point[_1D]) => Vector(2f) * x(0)))

      val domain = BoxDomain[_1D](-1.0f, 1.0f)
    ...
    it("Correctly integrates a compact function") {
      ...
      val img = ContinuousScalarImage1D(BoxDomain[_1D](-1.0f, 1.0f), (x: Point[_1D]) => 1.0)
