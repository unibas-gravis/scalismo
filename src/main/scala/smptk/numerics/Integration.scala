...
trait Sampler[D <: Dim] {  
  def sample(boxedRegion: BoxedRegion[D], numberOfPoints: Int): IndexedSeq[Point[D]]
}
...
case class UniformSampler1D extends UniformSampler[OneD] {
  def sample(boxedRegion: BoxedRegion[OneD], numberOfPoints: Int = 300) = {
...
case class UniformSampler2D extends UniformSampler[TwoD] {
  def sample(region: BoxedRegion[TwoD], numberOfPoints: Int = 300) = {
...
case class UniformSampler3D extends UniformSampler[ThreeD] {
  def sample(region: BoxedRegion[ThreeD], numberOfPoints: Int = 300) = {
...
case class UniformDistributionRandomSampler1D extends RandomSampler[OneD] {
  def sample(region: BoxedRegion[OneD], numberOfPoints: Int = 300) = {
...
case class Integrator[D <: Dim](configuration: IntegratorConfiguration[D]) {
  ...
  def integrateScalar(img: ContinuousScalarImage[D], integrationRegion: BoxedRegion[D]): Double = {
    integrateScalar(img.liftPixelValue, integrationRegion)
  }

  def integrateScalar(f: Function1[Point[D], Option[Double]], integrationRegion: BoxedRegion[D]): Double = {
  ...
  def integrateVector(img: ContinuousVectorImage[D], integrationRegion: BoxedRegion[D]): DenseVector[Double] = {
