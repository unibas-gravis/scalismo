...
//    ) {
//      val (x,y) = img.domain.linearIndexToIndex(ptNumber) 
//      betaMat(ptNumber, img.domain.indexToLinearIndex((i, j))) = splineBasis(x - i, y - j)
//    }
...
//  def interpolate(degree: Int)(image: DiscreteScalarImage2D): ContinuousScalarImage2D = {
//    val ck = determineCoefficients(degree, image) //image.pixelValues //DenseVector.ones[Float](image.domain.size(0))
//
//  }
