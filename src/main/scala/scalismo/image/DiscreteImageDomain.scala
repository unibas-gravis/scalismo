...
object DiscreteImageDomain {
  ...
  def apply[D](origin: Point[D], spacing: Vector[D], size: IntVector[D])(implicit evCreate: CreateDiscreteImageDomain[D]) = {
  ...
  /** Create a new discreteImageDomain with given image box (i.e. a box that determines the area where the image is defined) and size */
  def apply[D : NDSpace](imageBox: BoxDomain[D], spacing: Vector[D])(implicit evCreate: CreateDiscreteImageDomain[D]): DiscreteImageDomain[D] = {
