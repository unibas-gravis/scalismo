...
abstract class DiscreteImageDomain[D <: Dim: NDSpace] extends DiscreteDomain[D] with Equals {
  ...
  private def continuousIndextoIndex(cidx: SpatialVector[D]): IntVector[D] = {
    var d = 0
  ...
  private def pointToContinuousIndex(pt: Point[D]): SpatialVector[D] = {
    val data = (0 until dimensionality).map(i => (pt(i) - origin(i)) / spacing(i))
    SpatialVector[D](data.toArray)
  }

  def indexToPoint(i: IntVector[D]) = indexToPhysicalCoordinateTransform(Point[D](i.toArray.map(_.toDouble)))

  private def isIndex(continuousIndex: SpatialVector[D]): Boolean = (0 until dimensionality).forall(i => continuousIndex(i) - Math.round(continuousIndex(i)) < 1e-8)
...
object DiscreteImageDomain {
  ...
  def apply[D <: Dim](origin: Point[D], spacing: SpatialVector[D], size: IntVector[D])(implicit evCreate: CreateDiscreteImageDomain[D]) = {
  ...
  /** Create a new discreteImageDomain with given image box (i.e. a box that determines the area where the image is defined) and size */
  def apply[D <: Dim : NDSpace](imageBox: BoxDomain[D], spacing: SpatialVector[D])(implicit evCreate: CreateDiscreteImageDomain[D]): DiscreteImageDomain[D] = {
...
case class DiscreteImageDomain1D(size: IntVector[_1D], indexToPhysicalCoordinateTransform: AnisotropicSimilarityTransformation[_1D]) extends DiscreteImageDomain[_1D] {
  ...
  private val iVecImage: SpatialVector1D = indexToPhysicalCoordinateTransform(Point(1)) - indexToPhysicalCoordinateTransform(Point(0))
  override val spacing = SpatialVector1D(iVecImage.norm.toFloat)
  ...
  override def boundingBox: BoxDomain[_1D] = BoxDomain(origin, origin + SpatialVector(size(0) * spacing(0)))
...
case class DiscreteImageDomain2D(size: IntVector[_2D], indexToPhysicalCoordinateTransform: AnisotropicSimilarityTransformation[_2D]) extends DiscreteImageDomain[_2D] {
  ...
  private val iVecImage: SpatialVector2D = indexToPhysicalCoordinateTransform(Point(1, 0)) - indexToPhysicalCoordinateTransform(Point(0, 0))
  private val jVecImage: SpatialVector2D = indexToPhysicalCoordinateTransform(Point(0, 1)) - indexToPhysicalCoordinateTransform(Point(0, 0))

  override val directions = SquareMatrix[_2D]((iVecImage * (1.0 / iVecImage.norm)).toArray ++ (jVecImage * (1.0 / jVecImage.norm)).toArray)
  override val spacing = SpatialVector2D(iVecImage.norm.toFloat, jVecImage.norm.toFloat)
  ...
  override def boundingBox: BoxDomain[_2D] = {
    ...
    val extent = SpatialVector[_2D](extendData.toArray)
...
case class DiscreteImageDomain3D(size: IntVector[_3D], indexToPhysicalCoordinateTransform: AnisotropicSimilarityTransformation[_3D]) extends DiscreteImageDomain[_3D] {
  ...
  override val spacing = SpatialVector3D(positiveScalingParameters(0), positiveScalingParameters(1), positiveScalingParameters(2))
  ...
  private val iVecImage: SpatialVector3D = indexToPhysicalCoordinateTransform(Point(1, 0, 0)) - indexToPhysicalCoordinateTransform(Point(0, 0, 0))
  private val jVecImage: SpatialVector3D = indexToPhysicalCoordinateTransform(Point(0, 1, 0)) - indexToPhysicalCoordinateTransform(Point(0, 0, 0))
  private val kVecImage: SpatialVector3D = indexToPhysicalCoordinateTransform(Point(0, 0, 1)) - indexToPhysicalCoordinateTransform(Point(0, 0, 0))
...
sealed trait CreateDiscreteImageDomain[D <: Dim] {
  def createImageDomain(origin: Point[D], spacing: SpatialVector[D], size: IntVector[D]): DiscreteImageDomain[D]
...
object CreateDiscreteImageDomain {
  ...
  implicit object CreateDiscreteImageDomain1D extends CreateDiscreteImageDomain[_1D] {
    override def createImageDomain(origin: Point[_1D], spacing: SpatialVector[_1D], size: IntVector[_1D]): DiscreteImageDomain[_1D] = {
  ...
  implicit object CreateDiscreteImageDomain2D extends CreateDiscreteImageDomain[_2D] {
    override def createImageDomain(origin: Point[_2D], spacing: SpatialVector[_2D], size: IntVector[_2D]): DiscreteImageDomain[_2D] = {
      val rigidParameters = origin.toArray ++ Array(0.0)
      val anisotropicScalingParameters = spacing.toArray
  ...
  implicit object CreateDiscreteImageDomain3D extends CreateDiscreteImageDomain[_3D] {
    override def createImageDomain(origin: Point[_3D], spacing: SpatialVector[_3D], size: IntVector[_3D]): DiscreteImageDomain[_3D] = {
