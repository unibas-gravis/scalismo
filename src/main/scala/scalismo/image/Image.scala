 ...
 */
class DifferentiableScalarImage[D <: Dim: NDSpace](_domain: Domain[D], _f: Point[D] => Float, val df: Point[D] => SpatialVector[D]) extends ScalarImage[D](_domain, _f) {
  ...
  override def convolve(filter: Filter[D], numberOfPointsPerDim: Int)(implicit c: CreateDiscreteImageDomain[D]): DifferentiableScalarImage[D] = {
    ...
    def intermediateDF(imageX: Point[D])(t: Point[D]): Option[SpatialVector[D]] = {
      val p = (imageX - t).toPoint
      if (this.isDefinedAt(p)) Some(df(p) * filter(t)) else None
    }

    def convolvedImgDerivative(imageX: Point[D]): SpatialVector[D] = {
...
object DifferentiableScalarImage {
   ...
   */
  def apply[D <: Dim: NDSpace](domain: Domain[D], f: Point[D] => Float, df: Point[D] => SpatialVector[D]) = new DifferentiableScalarImage[D](domain, f, df)
