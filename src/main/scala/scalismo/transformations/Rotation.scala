...
object Rotation {
  ...
  /**
   * Factory method to create a 3-dimensional rotation transform around a center when given the Euler angles according
   * to the x-convention
   *
   * @param phi
...
case class Rotation3D(val rotationMatrix: SquareMatrix[_3D], val center: Point[_3D]) extends Rotation[_3D] {
  ...
  override def derivativeWRTParameters: JacobianField[_3D] = {
    val df = (x: Point[_3D]) => {
      ...
      val dr00 =
        (-sinphi * costh * x0minc0) + (-sinphi * sinpsi * sinth - cospsi * cosphi) * x1minc1 + (sinpsi * cosphi - cospsi * sinth * sinphi) * x2minc2
      val dr01 = -sinth * cosphi * x0minc0 + costh * sinpsi * cosphi * x1minc1 + cospsi * costh * cosphi * x2minc2
      val dr02 =
        (cospsi * sinth * cosphi + sinpsi * sinphi) * x1minc1 + (cospsi * sinphi - sinpsi * sinth * cosphi) * x2minc2

      val dr10 =
        costh * cosphi * x0minc0 + (-sinphi * cospsi + sinpsi * sinth * cosphi) * x1minc1 + (cospsi * sinth * cosphi + sinpsi * sinphi) * x2minc2
      val dr11 = -sinth * sinphi * x0minc0 + sinpsi * costh * sinphi * x1minc1 + cospsi * costh * sinphi * x2minc2
      val dr12 =
        (-sinpsi * cosphi + cospsi * sinth * sinphi) * x1minc1 + (-sinpsi * sinth * sinphi - cospsi * cosphi) * x2minc2
...
abstract class RotationSpace[D: NDSpace] extends TransformationSpaceWithDifferentiableTransforms[D] {
   ...
   *
   * Rotation parameters for _2D : a scalar indicating the rotation angle in radians Rotation parameters for _3D : Euler
   * angles in x-convention (rotation around Z axis first, around Y axis second, around X axis third)
   */
...
object RotationSpace {
  ...
  /**
   * Factory method to create a D dimensional parametric transformation space generating rotations around the indicated
   * centre Only _2D and _3D dimensions are supported
   */
