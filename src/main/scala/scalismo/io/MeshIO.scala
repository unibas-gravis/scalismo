...
object MeshIO {
  ...
  def readPolyLine2D(file: File): Try[PolyLine[_2D]] = {
    val filename = file.getAbsolutePath
    filename match {
  ...
  def readPolyLine3D(file: File): Try[PolyLine[_3D]] = {
    val filename = file.getAbsolutePath
    filename match {
      case f if f.endsWith(".vtk") => readLineMeshVTK(file)
      case _ =>
  ...
  def writePolyLine[D <: Dim](polyLine: PolyLine[D], file: File): Try[Unit] = {
    val filename = file.getAbsolutePath
    filename match {
  ...
  private def readLineMeshVTK[D <: Dim: NDSpace: PolyLine.Create: UnstructuredPointsDomain.Create](file: File): Try[PolyLine[D]] = {
    val vtkReader = new vtkPolyDataReader()
    vtkReader.SetFileName(file.getAbsolutePath)
    vtkReader.Update()
    val errCode = vtkReader.GetErrorCode()
    ...
    val correctedMesh = for {
      ...
      PolyLine.enforceConsistentCellDirections[D](polyline)
    }
    vtkReader.Delete()
    vtkPd.Delete()
    correctedMesh
  ...
  private[this] def writeLineMeshVTK[D <: Dim](mesh: PolyLine[D], file: File): Try[Unit] = {
    val vtkPd = MeshConversion.polyLineToVtkPolyData(mesh)
    val err = writeVTKPdasVTK(vtkPd, file)
    vtkPd.Delete()
    err
  }
