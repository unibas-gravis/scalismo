...
object StatisticalModelIO {
  ...
  /**
   * Reads and write of point distribution models with different domain types. The model file type is determined based
   * on the extension. Currently only the Scalismo format (.h5) is supported.
   */
  /**
  ...
  def readStatisticalTriangleMeshModel2D(file: File,
                                         modelPath: String = "/"
  ): Try[PointDistributionModel[_2D, TriangleMesh]] = {
  ...
  def readStatisticalTriangleMeshModel3D(file: File,
                                         modelPath: String = "/"
  ): Try[PointDistributionModel[_3D, TriangleMesh]] = {
  ...
  def readStatisticalLineMeshModel2D(file: File,
                                     modelPath: String = "/"
  ): Try[PointDistributionModel[_2D, LineMesh]] = {
  ...
  def readStatisticalLineMeshModel3D(file: File,
                                     modelPath: String = "/"
  ): Try[PointDistributionModel[_3D, LineMesh]] = {
   ...
   * @return
   *   PointDistributionModel with a TriangleMesh[_3D] as the domain
   */
  ...
  /**
   * Reads a statistical mesh model. The file type is determined based on the extension. Currently on the Scalismo
   * format (.h5) is supported.
   *
   * @param file
   *   The statismo file
   * @return
  ...
  /**
   * Writes a statistical mesh model. The file type is determined based on the extension. Currently on the Scalismo
   * format (.h5) is supported.
   *
   * @param model
   ...
   *   the file to which the model is written
   * @return
   *   A try, where the success case holds the model, and the failure case the corresponding exception
   */
