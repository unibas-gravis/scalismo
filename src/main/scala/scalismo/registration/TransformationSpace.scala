...
trait DifferentiableTransforms[D] { self: TransformationSpace[D] =>
  ...
  def product(that: TransformationSpace[D] with DifferentiableTransforms[D]) = {
    new DifferentiableProductTransformationSpace(this, that)
  }
}
 ...
 */
class DifferentiableProductTransformationSpace[D, OT <: ParametricTransformation[D] with CanDifferentiate[D], IT <: ParametricTransformation[
  D
] with CanDifferentiate[D]] private[registration] (outer: TransformationSpace[D] with DifferentiableTransforms[D],
                                                   inner: TransformationSpace[D] with DifferentiableTransforms[D])
    extends TransformationSpace[D]
    with DifferentiableTransforms[D] {

  override type T = DifferentiableCompositeTransformation[D]

  def parametersDimensionality = outer.parametersDimensionality + inner.parametersDimensionality

  def identityTransformParameters =
    DenseVector.vertcat(outer.identityTransformParameters, inner.identityTransformParameters)
  ...
  override def transformForParameters(p: ParameterVector) = {
    ...
    new DifferentiableCompositeTransformation(outer.transformForParameters(outerParams),
                                              inner.transformForParameters(innerParams))
  }
  ...
  override def takeDerivativeWRTParameters(p: ParameterVector) = {
    ...
    (x: Point[D]) => {
      DenseMatrix.horzcat(
        ...
        outerTransform.takeDerivative(inner.transformForParameters(innerParams)(x)).toBreezeMatrix * inner
          .takeDerivativeWRTParameters(innerParams)(x)
      )
    }
  }
...
object DifferentiableProductTransformationSpace {
  def apply[D, OT <: ParametricTransformation[D] with CanDifferentiate[D], IT <: ParametricTransformation[D] with CanDifferentiate[
    D
  ]](outer: TransformationSpace[D] with DifferentiableTransforms[D],
     inner: TransformationSpace[D]
       with DifferentiableTransforms[D]): DifferentiableProductTransformationSpace[D, OT, IT] = {
    new DifferentiableProductTransformationSpace(outer, inner)
  }
}
 ...
 * @tparam IT Type of the inner transformation space. This space must generate differentiable parametric transforms
 */
class ProductTransformationSpace[D, OT <: ParametricTransformation[D] with CanDifferentiate[D], IT <: ParametricTransformation[
  D
]] private[registration] (outer: TransformationSpace[D] with DifferentiableTransforms[D], inner: TransformationSpace[D])
    extends TransformationSpace[D] {
...
object ProductTransformationSpace {
  def apply[D, OT <: ParametricTransformation[D] with CanDifferentiate[D], IT <: ParametricTransformation[D]](
    outer: TransformationSpace[D] with DifferentiableTransforms[D],
    inner: TransformationSpace[D]
  ): ProductTransformationSpace[D, OT, IT] = {
    new ProductTransformationSpace[D, OT, IT](outer, inner)
  }
}
 ...
 */
case class DifferentiableCompositeTransformation[D](
  outerTransform: ParametricTransformation[D] with CanDifferentiate[D],
  innerTransform: ParametricTransformation[D] with CanDifferentiate[D]
 ...
 *  @param outerTransform transform to be applied second. Must be a parametric differentiable transform
 */
case class CompositeTransformation[D](outerTransform: ParametricTransformation[D],
                                      innerTransform: ParametricTransformation[D])
    extends ParametricTransformation[D] {
...
@deprecated("This class will be removed in the next version of scalismo. Please use CompositeTransformation instead.",
            "0.15.0")
case class ProductTransformation[D](outerTransform: ParametricTransformation[D] with CanDifferentiate[D],
                                    innerTransform: ParametricTransformation[D] with CanDifferentiate[D])
    extends ParametricTransformation[D]
    with CanDifferentiate[D] {

  private val compositeTransformation = DifferentiableCompositeTransformation(outerTransform, innerTransform)
...
class TranslationSpace[D: NDSpace] private () extends TransformationSpace[D] with DifferentiableTransforms[D] {
  ...
  override def transformForParameters(p: ParameterVector): TranslationTransform[D] =
    TranslationTransform[D](EuclideanVector[D](p.data))

  override def takeDerivativeWRTParameters(p: ParameterVector) = { x: Point[D] =>
    DenseMatrix.eye[Double](parametersDimensionality)
  }
}
 ...
 */
case class TranslationTransform[D: NDSpace](t: EuclideanVector[D])
    extends ParametricTransformation[D]
    with CanInvert[D]
    with CanDifferentiate[D] {
...
private class RotationSpace1D extends RotationSpace[_1D] {
  ...
  override def takeDerivativeWRTParameters(p: ParameterVector) = { (x: Point[_1D]) =>
...
private class RotationSpace2D(val centre: Point[_2D]) extends RotationSpace[_2D] {
  ...
  override def transformForParameters(p: ParameterVector): RotationTransform[_2D] = {
    ...
    val rotMatrix = SquareMatrix((math.cos(p(0)), -math.sin(p(0))), (math.sin(p(0)), math.cos(p(0))))
 ...
 */
abstract class RotationTransform[D: NDSpace]
    extends ParametricTransformation[D]
    with CanInvert[D]
    with CanDifferentiate[D] {
...
private case class RotationTransform3D(rotMatrix: SquareMatrix[_3D], val center: Point[_3D])
    extends RotationTransform[_3D] {
...
private case class RotationTransform2D(rotMatrix: SquareMatrix[_2D], val center: Point[_2D])
    extends RotationTransform[_2D] {
...
object RotationTransform {
  ...
  implicit object createRotationTransform2D extends Create[_2D] {
    override def createRotationTransform(rotMatrix: SquareMatrix[_2D], centre: Point[_2D]): RotationTransform[_2D] =
  ...
  implicit object createRotationTransform3D extends Create[_3D] {
    override def createRotationTransform(rotMatrix: SquareMatrix[_3D], centre: Point[_3D]): RotationTransform[_3D] =
  ...
  def apply(phi: Double, center: Point[_2D]): RotationTransform[_2D] = {
    ...
    val rotMatrix = SquareMatrix((math.cos(phi), -math.sin(phi)), (math.sin(phi), math.cos(phi)))
...
private class ScalingSpace3D extends ScalingSpace[_3D] {
  override def takeDerivativeWRTParameters(p: ParameterVector) = { x: Point[_3D] =>
    DenseMatrix(x(0), x(1), x(2))
  }
}

private class ScalingSpace2D extends ScalingSpace[_2D] {
  override def takeDerivativeWRTParameters(p: ParameterVector) = { x: Point[_2D] =>
    DenseMatrix(x(0), x(1))
  }
}
 ...
 */
class ScalingTransformation[D: NDSpace] private (s: Double)
    extends ParametricTransformation[D]
    with CanInvert[D]
    with CanDifferentiate[D] {
...
class RigidTransformationSpace[D: NDSpace: CreateRotationSpace] private (center: Point[D])
    extends DifferentiableProductTransformationSpace[D, TranslationTransform[D], RotationTransform[D]](
      TranslationSpace[D],
      RotationSpace[D](center)
    ) {
  ...
  override def transformForParameters(p: ParameterVector): RigidTransformation[D] = {
    val (outerParams, innerParams) = splitProductParameterVector(p)
    new RigidTransformationTranslationFollowingRotation[D](TranslationSpace[D].transformForParameters(outerParams),
                                                           RotationSpace[D](center).transformForParameters(innerParams))
  }
}
 ...
 */
trait RigidTransformation[D] extends DifferentiableCompositeTransformation[D] with CanInvert[D] {
...
object RigidTransformation {
  ...
  def apply[D: NDSpace](translationTransform: TranslationTransform[D],
                        rotationTransform: RotationTransform[D]): RigidTransformation[D] =
  ...
  def apply[D: NDSpace](rotationTransform: RotationTransform[D],
                        translationTransform: TranslationTransform[D]): RigidTransformation[D] =
    new RigidTransformationRotationFollowingTranslation(rotationTransform, translationTransform)

}

private class RigidTransformationTranslationFollowingRotation[D: NDSpace](val translation: TranslationTransform[D],
                                                                          val rotation: RotationTransform[D])
    extends DifferentiableCompositeTransformation[D](translation, rotation)
    with RigidTransformation[D] {

  def inverse = new RigidTransformationRotationFollowingTranslation[D](rotation.inverse, translation.inverse)
}

private class RigidTransformationRotationFollowingTranslation[D: NDSpace](val rotation: RotationTransform[D],
                                                                          val translation: TranslationTransform[D])
    extends DifferentiableCompositeTransformation[D](rotation, translation)
    with RigidTransformation[D] {
  def inverse = new RigidTransformationTranslationFollowingRotation[D](translation.inverse, rotation.inverse)

}

trait SimilarityTransformation[D] extends DifferentiableCompositeTransformation[D] with CanInvert[D] {
...
object SimilarityTransformation {
   ...
   * @return
   */
  def apply[D: NDSpace](scaling: ScalingTransformation[D],
                        rigidTransformation: RigidTransformation[D]): SimilarityTransformation[D] = {
    new SimilarityTransformationScalingFollowingRigid[D](scaling, rigidTransformation)
  }
  def apply[D: NDSpace](rigidTransformation: RigidTransformation[D],
                        scaling: ScalingTransformation[D]): SimilarityTransformation[D] = {
...
private class SimilarityTransformationScalingFollowingRigid[D: NDSpace](val scaling: ScalingTransformation[D],
    ...
    extends DifferentiableCompositeTransformation[D](scaling, rigidTransformation)
    with SimilarityTransformation[D] {

  override def inverse: SimilarityTransformation[D] =
    new SimilarityTransformationRigidFollowingScaling[D](rigidTransformation.inverse, scaling.inverse)
}
...
private class SimilarityTransformationRigidFollowingScaling[D: NDSpace](val rigidTransformation: RigidTransformation[D],
    ...
    extends DifferentiableCompositeTransformation[D](rigidTransformation, scaling)
    with SimilarityTransformation[D] {

  override def inverse: SimilarityTransformation[D] =
    new SimilarityTransformationScalingFollowingRigid(scaling.inverse, rigidTransformation.inverse)
}
...
case class SimilarityTransformationSpace[D: NDSpace: CreateRotationSpace: ScalingSpace.Create](center: Point[D])
    extends DifferentiableProductTransformationSpace[D, ScalingTransformation[D], RigidTransformation[D]](
      ScalingSpace[D](),
      RigidTransformationSpace[D](center)
    ) {
...
case class AnisotropicScalingSpace[D: NDSpace]() extends TransformationSpace[D] with DifferentiableTransforms[D] {
  ...
  override def takeDerivativeWRTParameters(p: ParameterVector) = { x: Point[D] =>
    new DenseMatrix(parametersDimensionality, 1, x.toArray)
  }
}
 ...
 *
 */
trait AnisotropicSimilarityTransformation[D] extends DifferentiableCompositeTransformation[D] with CanInvert[D] {
...
private class RigidTransformationThenAnisotropicScaling[D: NDSpace](
  ...
  rigidTransform: RigidTransformation[D]
    ...
    with AnisotropicSimilarityTransformation[D] {

  override def inverse: AnisotropicScalingThenRigidTransformation[D] =
...
private class AnisotropicScalingThenRigidTransformation[D: NDSpace](
  ...
  override def inverse: RigidTransformationThenAnisotropicScaling[D] =
...
case class AnisotropicSimilarityTransformationSpace[D: NDSpace: CreateRotationSpace](center: Point[D])
    extends DifferentiableProductTransformationSpace[D, RigidTransformation[D], AnisotropicScalingTransformation[D]](
      RigidTransformationSpace[D](center),
      AnisotropicScalingSpace[D]()
    ) {
  ...
  override def transformForParameters(p: ParameterVector): AnisotropicSimilarityTransformation[D] = {
    ...
    val rigid = RigidTransformationSpace[D](center)
      .transformForParameters(rigidP)
      .asInstanceOf[RigidTransformationTranslationFollowingRotation[D]]
    new AnisotropicScalingThenRigidTransformation[D](rigid,
                                                     AnisotropicScalingSpace[D]().transformForParameters(scalingP))
