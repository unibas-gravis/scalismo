...
import scalismo.geometry.{ Dim, Point, EuclideanVector }
...
class GaussianProcessTransformationSpace[D <: Dim] private(gp: LowRankGaussianProcess[D, EuclideanVector[D]])(implicit vectorizer: VectorVectorizer[D])
...
class GaussianProcessTransformation[D <: Dim] private (gp: LowRankGaussianProcess[D, EuclideanVector[D]], alpha: ParameterVector)
...
object GaussianProcessTransformation {
  def apply[D <: Dim](gp: LowRankGaussianProcess[D, EuclideanVector[D]], alpha: TransformationSpace.ParameterVector) = {
    new GaussianProcessTransformation[D](gp, alpha)
  }
}

object GaussianProcessTransformationSpace {
  def apply[D <: Dim](gp: LowRankGaussianProcess[D, EuclideanVector[D]])(implicit vectorizer: VectorVectorizer[D]) = {
