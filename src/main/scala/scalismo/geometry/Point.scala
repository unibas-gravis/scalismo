...
sealed abstract class Point[D: NDSpace] {
  ...
  def +(that: EuclideanVector[D]): Point[D]

  def -(that: EuclideanVector[D]): Point[D]

  def -(that: Point[D]): EuclideanVector[D]

  def toVector: EuclideanVector[D]
...
case class Point1D(x: Double) extends Point[_1D] {
  ...
  override def +(that: EuclideanVector[_1D]): Point1D = Point1D(x + that.x)

  override def -(that: EuclideanVector[_1D]): Point1D = Point1D(x - that.x)

  override def -(that: Point[_1D]): EuclideanVector1D = EuclideanVector1D(x - that.x)

  override def toVector: EuclideanVector1D = EuclideanVector1D(x)
...
case class Point2D(x: Double, y: Double) extends Point[_2D] {
  ...
  override def +(that: EuclideanVector[_2D]): Point2D = Point2D(x + that.x, y + that.y)

  override def -(that: EuclideanVector[_2D]): Point2D = Point2D(x - that.x, y - that.y)

  override def -(that: Point[_2D]): EuclideanVector2D = EuclideanVector2D(x - that.x, y - that.y)

  override def toVector: EuclideanVector2D = EuclideanVector2D(x, y)
...
case class Point3D(x: Double, y: Double, z: Double) extends Point[_3D] {
  ...
  override def +(that: EuclideanVector[_3D]): Point3D = Point3D(x + that.x, y + that.y, z + that.z)

  override def -(that: EuclideanVector[_3D]): Point3D = Point3D(x - that.x, y - that.y, z - that.z)

  override def -(that: Point[_3D]): EuclideanVector3D = EuclideanVector3D(x - that.x, y - that.y, z - that.z)

  override def toVector: EuclideanVector[_3D] = EuclideanVector3D(x, y, z)
