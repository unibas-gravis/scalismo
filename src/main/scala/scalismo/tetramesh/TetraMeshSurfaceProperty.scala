 ...
 */
trait TetraMeshSurfaceProperty[A] {
  /** access property value on surface, via coordinates in triangle */
  def onSurface(triangleId: TetrahedronId, bcc: BarycentricCoordinates): A

  /** access property value on surface, via coordinates in triangle */
  def apply(triangleId: TetrahedronId, bcc: BarycentricCoordinates): A = onSurface(triangleId, bcc)

  /** apply function f to the surface property values */
  def map[B](f: A => B): TetraMeshSurfaceProperty[B] = MappedSurfaceProperty(this, f)

  /** triangulation: domain of surface property (parameterization of surface) */
  def tetrahedriangulation: TetrahedralList
}

/** constant property value on complete surface */
case class ConstantProperty[A](triangulation: TetrahedralList, value: A)
    extends TetraMeshSurfaceProperty[A] {

  override def onSurface(triangleId: TetrahedronId, bcc: BarycentricCoordinates): A = value
...
case class SurfacePointProperty[A](triangulation: TetrahedralList, pointData: IndexedSeq[A])(implicit val interpolator: ValueInterpolator[A])
    extends TetraMeshSurfaceProperty[A] {
  ...
  override def onSurface(triangleId: TetrahedronId, bcc: BarycentricCoordinates): A = {
