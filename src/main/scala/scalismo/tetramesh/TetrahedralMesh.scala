...
case class TetrahedralCell(ptId1: PointId, ptId2: PointId, ptId3: PointId,ptId4: PointId) extends Cell {
  ...
  def toIntVector4D = IntVector(ptId1.id, ptId2.id, ptId3.id,ptId4.id)
}

trait TetrahedralMesh[D] {
  def triangulation: TetrahedralList
...
object TetrahedralMesh {
  ...
  trait Create2D extends Create[_2D] {
    override def createTetrahedraleMesh(pointSet: UnstructuredPointsDomain[_2D], topology: TetrahedralList) = {
  ...
  implicit def parametricToConcreteType2D(triangleMesh: TetrahedralMesh[_2D]): TriangleMesh2D = {
    triangleMesh.asInstanceOf[TriangleMesh2D]
  }

  implicit def parametricToConcreteType3D(triangleMesh: TetrahedralMesh[_3D]): TriangleMesh3D = {
...
object TriangleMesh3D {
  def apply(points: IndexedSeq[Point[_3D]], topology: TetrahedralList): TriangleMesh3D = {
    TriangleMesh3D(UnstructuredPointsDomain(points.toIndexedSeq), topology)
  }
}

case class TetrahedralMesh2D(pointSet: UnstructuredPointsDomain[_2D], tetrahedriangulation: TetrahedralList) extends TetrahedralMesh[_2D] {
  val position = SurfacePointProperty(tetrahedriangulation, pointSet.points.toIndexedSeq)

  override def transform(transform: Point[_2D] => Point[_2D]): TetrahedralMesh2D = {
    TetrahedralMesh2D(UnstructuredPointsDomain(pointSet.points.map(transform).toIndexedSeq), triangulation)
  }
}
