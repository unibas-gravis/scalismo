...
class MeshDecimation(mesh: TriangleMesh[_3D]) {
  ...
  var refs: IndexedSeq[Ref] = mesh.triangulation.triangleIds.flatMap { tid =>
    ...
    ).map(ptId => Ref(tid.id, ptId.id))
  }

  def simplify(targetCount: Int, agressiveness: Double): TriangleMesh[_3D] = {
    var deletedTriangles = 0

    val deleted0: Vector[Boolean] = Vector.empty[Boolean]
    val deleted1: Vector[Boolean] = Vector.empty[Boolean]

    var triangleCount = numberOfTriangles
    ...
    breakable {
      ...
      (0 until 10).foreach { iteration =>
        ...
        val threshold: Double = 0.000000001 * Math.pow(iteration + 3, agressiveness)
...
//                    break
  ...
  private def updateTriangles(i0: Int, v: Vertex, deleted: Array[Boolean]): Int = {
    ...
    (0 until v.tcount).foreach { k =>
      ...
      if (!t.deleted) {
        if (!deleted(k)) {
          triangles = triangles.updated(r.tid, triangles(r.tid).copy(deleted = true))
          triangleDeleted += 1
        } else {
    ...
    if (iteration == 0) {
      triangles.zipWithIndex.foreach { case (t, i) =>
        (0 until 3).foreach { j =>
          ...
          val newQ =
            vertices(id).q.add(new SymmetricMatrix(t.n.x, t.n.y, t.n.z, -t.n.dot(vertices(t.v.ptId1.id).p.toVector)))
          vertices = vertices.updated(id, vertices(id).copy(q = newQ))
        }
      }
    } else if (iteration > 0) {
      var distance = 0
  ...
  private def compactMesh(): Unit = {
    ...
    triangles.foreach { t =>
      if (!t.deleted) {
        ...
        (0 until 3).foreach { j =>
          val id = t.v.pointIds(j).id;
          vertices = vertices.updated(id, vertices(id).copy(tcount = 1))
        }
      }
    }
    triangles = triangles.take(distance)
  ...
  def calculateError(v1: Vertex, v2: Vertex): (Double, EuclideanVector3D) = {
    ...
    val (error, p) = if (det != 0 && !border) {
      ...
      val vect = if (error1 == error) p1 else if (error2 == error) p2 else p3
      (error, vect)
    }
    (error, p)
  }
