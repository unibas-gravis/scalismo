...
import scalismo.geometry.{ Dim, Point, SpatialVector, _3D }
...
trait SurfaceIntersectionIndex[D <: Dim] {
  ...
  def hasIntersection(point: Point[D], direction: SpatialVector[D]): Boolean

  def getIntersectionPoints(point: Point[D], direction: SpatialVector[D]): Seq[Point[D]]
...
trait TriangulatedSurfaceIntersectionIndex[D <: Dim] extends SurfaceIntersectionIndex[D] {
  ...
  def getSurfaceIntersectionPoints(point: Point[D], direction: SpatialVector[D]): Seq[(TriangleId, BarycentricCoordinates)]
}
...
private[mesh] class LineTriangleMesh3DIntersectionIndex(private val boundingSphere: BoundingSphere,
  ...
  override def hasIntersection(point: Point[_3D], direction: SpatialVector[_3D]): Boolean = {
    intersectWithLine(point.toVector, direction, boundingSphere).nonEmpty
  }

  override def getIntersectionPoints(point: Point[_3D], direction: SpatialVector[_3D]): Seq[Point[_3D]] = {
    intersectWithLine(point.toVector, direction, boundingSphere)
  }

  override def getSurfaceIntersectionPoints(point: Point[_3D], direction: SpatialVector[_3D]): Seq[(TriangleId, BarycentricCoordinates)] = {
    surfaceIntersectionPoint(point.toVector, direction, boundingSphere).map(t => (TriangleId(t._1), t._2))
  }

  private def intersectWithLine(point: SpatialVector[_3D], direction: SpatialVector[_3D], partition: BoundingSphere): Seq[Point[_3D]] = {
  ...
  private def surfaceIntersectionPoint(point: SpatialVector[_3D], direction: SpatialVector[_3D], partition: BoundingSphere): List[(Int, BarycentricCoordinates)] = {
    if (BSIntersection.intersectLineSphereSquared(point, direction, partition.center, partition.r2)) {
      if (partition.idx < 0) {
        val l = if (partition.hasLeft) {
