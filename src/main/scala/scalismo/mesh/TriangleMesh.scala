...
case class TriangleMesh3D(pointSet: UnstructuredPointsDomain[_3D], triangulation: TriangleList) extends TriangleMesh[_3D] {
  ...
  lazy val cellNormals: TriangleProperty[SpatialVector[_3D]] = {
    val triangleNormals: Array[SpatialVector[_3D]] = new Array[SpatialVector[_3D]](triangles.size)
  ...
  lazy val vertexNormals: SurfacePointProperty[SpatialVector[_3D]] = {
    // create data array: average over all adjacent triangles
    val pointNormals = new Array[SpatialVector[_3D]](pointSet.numberOfPoints)
    pointSet.pointIds.foreach { ptId =>
      ...
      val n = tr.size
      pointNormals(ptId.id) = SpatialVector3D(x / n, y / n, z / n).normalize
    }
  ...
  def computeCellNormal(cell: TriangleCell): SpatialVector[_3D] = {
