 ...
 */
case class StatisticalMeshModel private (referenceMesh: TriangleMesh[_3D], gp: DiscreteLowRankGaussianProcess[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]]) {
  ...
  def project(mesh: TriangleMesh[_3D]) = {
    ...
    val dvf = DiscreteField[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]](referenceMesh.pointSet, displacements)
  ...
  def transform(rigidTransform: RigidTransformation[_3D]): StatisticalMeshModel = {
    ...
    val newGp = new DiscreteLowRankGaussianProcess[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]](gp.domain.transform(rigidTransform), newMean, gp.variance, newBasisMat)
  ...
  def changeReference(t: Point[_3D] => Point[_3D]): StatisticalMeshModel = {
    ...
    val newGp = new DiscreteLowRankGaussianProcess[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]](newRef, newMeanVec, gp.variance, gp.basisMatrix)
    new StatisticalMeshModel(TriangleMesh3D(newRef, referenceMesh.triangulation), newGp)
  }

  private def warpReference(vectorPointData: DiscreteField[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]]) = {
...
object StatisticalMeshModel {
   ...
   */
  def apply(referenceMesh: TriangleMesh[_3D], gp: LowRankGaussianProcess[_3D, SpatialVector[_3D]]): StatisticalMeshModel = {
  ...
  private[scalismo] def apply(referenceMesh: TriangleMesh[_3D],
    ...
    val gp = new DiscreteLowRankGaussianProcess[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]](referenceMesh.pointSet, meanVector, variance, basisMatrix)
   ...
   *
   */
  def createUsingPCA(referenceMesh: TriangleMesh[_3D], fields: Seq[Field[_3D, SpatialVector[_3D]]]): StatisticalMeshModel = {
    val dgp: DiscreteLowRankGaussianProcess[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]] = DiscreteLowRankGaussianProcess.createUsingPCA(referenceMesh.pointSet, fields)
  ...
  def augmentModel(model: StatisticalMeshModel, biasModel: GaussianProcess[_3D, SpatialVector[_3D]], numBasisFunctions: Int)(implicit rand: Random): StatisticalMeshModel = {

    val modelGP = model.gp.interpolateNearestNeighbor
    // TODO: check if there is a better alternative (move method to Field?)
    val newMean = Field[_3D, SpatialVector[_3D]](modelGP.domain,
  ...
  def augmentModel(model: StatisticalMeshModel, biasModel: LowRankGaussianProcess[_3D, SpatialVector[_3D]]) = {
    ...
    val r = model.gp.copy[_3D, UnstructuredPointsDomain[_3D], SpatialVector[_3D]](meanVector = model.gp.meanVector + discretizedBiasModel.meanVector, variance = breeze.numerics.pow(d, 2), basisMatrix = U)
  ...
  def createUsingPCA(dc: DataCollection): Try[StatisticalMeshModel] = {
    ...
    val fields = dc.dataItems.map { i =>
      Field[_3D, SpatialVector[_3D]](i.transformation.domain, p => i.transformation(p) - p)
    }
