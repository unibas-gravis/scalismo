...
/**
 * A gaussian process which is represented in terms of a (small) finite set of basis functions. The basis functions are
 * the orthonormal basis functions given by a mercers' decomposition.
 *
 * @param mean
 *   The mean function
 * @param klBasis
 ...
 *   The output type
 */
class LowRankGaussianProcess[D: NDSpace, Value](mean: Field[D, Value], val klBasis: KLBasis[D, Value])(implicit
  ...
  /**
   * an instance of the gaussian process, which is formed by a linear combination of the klt basis using the given
   * coefficients c.
   *
   * @param c
  ...
  /**
   * Returns the sample of the gaussian process that best explains the given training data. It is assumed that the
   * training data (values) are subject to 0 mean Gaussian noise
   *
   * @param trainingData
   ...
   *
   * @param trainingData
   *   Point/value pairs where that the sample should approximate, together with the variance of the noise model at each
   *   point.
   */
  ...
  /**
   * Returns the sample of the coefficients of the sample that best explains the given training data. It is assumed that
   * the training data (values) are subject to 0 mean Gaussian noise
   */
...
/**
 * Factory methods for creating Low-rank gaussian processes, as well as generic algorithms to manipulate Gaussian
 * processes.
 */
object LowRankGaussianProcess {
  ...
  /**
   * Perform a low-rank approximation of the Gaussian process using the Nystrom method. The sample points used for the
   * nystrom method are sampled using the given sample.
   *
   * @param gp
  ...
  @deprecated("the method has been renamed to approximateGPNystrom", "0.17")
  def approximateGP[D: NDSpace, Value](gp: GaussianProcess[D, Value], sampler: Sampler[D])(implicit
    vectorizer: Vectorizer[Value],
    rand: Random
  ) = {
  ...
  /**
   * Perform a low-rank approximation of the Gaussian process using a pivoted Cholesky approximation. This approximation
   * will automatically compute the required number of basis functions, to achieve a given approximation quality.
   *
   * Note: The number of basis functions that are generated for a fixed approximation quality depends very much on the
   * smoothness of the process. The more smooth the modelled functions are, the less basis functions are required. For
   * models which are highly non-smooth, this method may generate a lot of basis functions.
   *
   * @param domain
   *   The domain on which the approximation is performed. This can, for example be the points of a mesh, an image
   *   domain (regular grid) or any other suitable domain. Note that the number of points in this domain influences the
   *   approximation accuracy. As the complexity of this method grows at most linearly with the number of points,
   *   efficient approximations can be computed for domains which contain millions of points.
   *
   * @param gp
   *   The gaussian process to be approximated
   *
   * @param relativeTolerance
   *   The relative tolerance defines the stopping criterion for the approximation. When we choose this parameter as
   *   0.01, the method will stop computing new basis functions, when the variance represented by the approximated
   *   Gaussian Process is 99% of the total variance of the original process. A value of 0 will generate new basis
   *   functions until all the variance is approximated. Note that this will only terminate when the Gaussian process
   *   has finite rank.
   *
   * @param interpolator
  ...
  /**
   * computes the log marginal likelihood given the eigendecomposition USUt and arbitrary independent noise on
   * observations
   *
   * @param U
   *   contains the relevant parts of the eigenfunctions
   * @param S
   *   contains the eigenvalues/variance
   * @param td
   *   non-empty seq that contains the noise defining the matrix A, where each entry is in order with the matrix U
   */
  private[scalismo] def marginalLikelihoodComputation(U: DenseMatrix[Double],
                                                      S: DenseVector[Double],
                                                      y: DenseVector[Double],
                                                      td: IndexedSeq[MultivariateNormalDistribution]
  ): Double = {
